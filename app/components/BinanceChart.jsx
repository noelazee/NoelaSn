...
export default function BinanceChart({
  ...
  const [chartH,    setChartH]      = useState(typeof height === 'number' ? height : 400)
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !containerRef.current || chartH < 60) return
    let destroyed = false
    ...
    const initChart = async () => {
      try {
        ...
        const chart = createChart(containerRef.current, {
          ...
          grid: {
            vertLines: { color: '#1c1c2a' },
            horzLines: { color: '#1c1c2a' },
          },
          crosshair:       { mode: 1 },
        ...
        const res  = await fetch(`/api/binance/klines?symbol=${symbol}&interval=${interval}&limit=150`)
        ...
        ws.onclose = () => {
          if (!destroyed) setTimeout(initChart, 3000)
        }
        ...
        const ro = new ResizeObserver(() => {
          ...
          const newH = typeof height === 'number'
            ? height
            : (containerRef.current.parentElement?.clientHeight || chartH) - 44
          chart.applyOptions({
            width:  newW,
            height: Math.max(newH, 60),
          })
        })
        ...
        console.error('[BinanceChart]', err)
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {status === 'loading' && (
            <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, color: '#4a5568' }}>Loading...</span>
          )}
          {status === 'error' && (
            <span
              onClick={() => setStatus('loading')}
              style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, color: '#ff3b5c', cursor: 'pointer' }}
            >
              ⚠ Retry
            </span>
          )}
